...
class Menu:
    ...
    def __init__(self):
        ...
        self.title_font = font.Font(
            "assets/fonts/NewAmsterdam-Regular.ttf", 64)
    ...
    def update(self):
        ...
        title_text = self.title_font.render("Down To The Roots", True, WHITE)
...
class CharacterSelect:
    """Game character select scene class
    """
    ...
    def __init__(self):
        ...
        self.background = image.load("assets/level/large_bg.png").convert()
        self.background = transform.scale(self.background, (WIDTH, HEIGHT))
        stickman_img = image.load("assets/chars_menu/stickman.png")
        ranger_img = image.load("assets/chars_menu/leaf_ranger.png")
        ranger_broke = image.load("assets/chars_menu/broken_leaf.png")
        fire_img = image.load("assets/chars_menu/fire_knight.png")
        fire_broken = image.load("assets/chars_menu/fire_broken.png")
        self.characters = [
            {
                "name": "Stickman", "description": "Weak and sick :)",
                "image": stickman_img
            },
            {
                "name": "Leaf Ranger", "description": "Wise and nimble",
                "image": ranger_img, "broken_image": ranger_broke
            },
            {
                "name": "Warrior", "description": "Strong and brave",
                "image": fire_img, "broken_image": fire_broken
            },
        ]
        ...
        self.troll_text = [
            "You can't handle this power!", "Not for you mortal!"]
    ...
    def update(self):
        ...
        title_text = self.title_font.render(
            "Select character that most similar to you", True, WHITE)
        title_rect = title_text.get_rect(center=(WIDTH / 2, 50))
        self.screen.blit(title_text, title_rect)
        for i, character in enumerate(self.characters):
            draw.rect(self.screen, (128, 128, 128),
                      self.character_buttons[i], 0)
            draw.rect(self.screen, (255, 255, 255),
                      self.character_buttons[i], 2)
            if self.broken_characters[i]:
                broken_image = transform.scale(
                    character.get("broken_image"), (200, 200))
                self.screen.blit(broken_image, self.character_buttons[i])
                text = self.font.render(self.troll_text[i-1], True, WHITE)
                text_rect = text.get_rect(
                    center=(self.character_buttons[i].centerx, self.character_buttons[i].top - 20))
                self.screen.blit(text, text_rect)
            else:
                char_image = transform.scale(
                    character.get("image"), (200, 200))
                self.screen.blit(char_image, self.character_buttons[i])
            name_text = self.font.render(character.get("name"), True, WHITE)
            name_rect = name_text.get_rect(center=(
                self.character_buttons[i].centerx, self.character_buttons[i].bottom + 40))
            self.screen.blit(name_text, name_rect)
            description_text = self.font.render(
                character.get("description"), True, WHITE)
            description_rect = description_text.get_rect(center=(
                self.character_buttons[i].centerx, self.character_buttons[i].bottom + 70))
